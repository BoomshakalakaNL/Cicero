...
beforeEach( async () => {
  ...
  await factory.methods.createDeclaration(accounts[4], accounts[2], "D3002;BB025;A3002", "13;3,15", "1680;1500;0100", "01-06-2018", "31-06-2018").send({
      from: accounts[0],
      gas: '3000000'
    });
...
describe('Declaration Contract', () => {
  ...
  it('The Care Administration Office can edit the declaration', async () => {
    await declaration.methods.editDeclaration(accounts[4], accounts[2], "TEST1;TEST2;TEST3", "12;3,15", "1234;5678;9012", "01-01-2000", "30-01-2000").send({
      from: accounts[0],
      gas: '3000000'
    });
    const summary = await declaration.methods.getDeclaration().call();
